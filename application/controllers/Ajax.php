...
class Ajax extends CI_Controller {
	...
	public function users_list(){
		// Datatables Variables
          ...
          $users_list = $this->admin_Model->get_where('member',$where);
          
          $data = array();

          foreach($users_list->result() as $user_list) {

              $more_content = (strlen($user_list->address) > 60) ? "..." : "";
              ...
              // }
               ...
               $data[] = array(
                    ...
                    $user_list->email,
                    //$hashtags,
                    '<div class="more">'.substr($user_list->address,0,60).$more_content.'</div>',
                    $action_data
               );
  ...
  public function hashtags_list(){
    // Datatables Variables
          ...
          foreach($hashtags_list->result() as $key => $hashtag_list) {
            $more_content = (strlen($hashtag_list->description) > 60) ? "..." : "";
               $action_data = '<a href="'.base_url().'hashtags/edit/'.$hashtag_list->id.'" class="on-default edit-row" data-toggle="tooltip" data-placement="top" title="" data-original-title="Edit"><i class="fa fa-lg fa-pencil"></i></a><a href="javascript:delete_single_record('."'$hashtag_list->id'".','."'hashtag'".','."'hashtag'".');" class="on-default remove-row danger-alert" data-toggle="tooltip" data-placement="top" title="" data-original-title="Delete"><i class="fa fa-lg fa-trash-o"></i></a>';  
               $data[] = array(
                    $hashtag_list->id,

                    "#".$hashtag_list->tag_name,
                    // '<div class="more">'.substr($hashtag_list->description,0,60).$more_content.'</div>',
  ...
  public function softdelete($table = '',$id = ''){
    $this->db->where('id',$id);
    $this->db->update($table, array( 'deleted_at' => date('Y-m-d H:i:s') ));

    $this->db->delete('member_extrainfo', array( 'member_id' => $id ));
